...
async function createShadowImage() {
  try {
    ...
    console.log('📝 Attempting alternative method using SVG...');
    
    // Alternative method using SVG
    createShadowImageSVG();
  }
}

// Fallback method to create SVG shadow
function createShadowImageSVG() {
  try {
    // Create an SVG with a strong radial gradient shadow
    const size = 100;
...
<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
  ...
  <circle cx="50" cy="50" r="50" fill="url(#shadowGradient)" />
    ...
    console.log(`📏 Image size: ${size}x${size} pixels`);
    console.log(`🎨 Gradient: 100% black center → transparent edge`);
    console.log(`💡 Note: Using SVG format (can be used directly in React)`);
