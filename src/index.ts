...
export class EphemerisCalculator {
  ...
  constructor(latitude = 0, longitude = 0, elevation = 0) {
    this.observer = new (Astronomy as any).Observer(latitude, longitude, elevation);
  }
  ...
  calculatePlanetEphemeris(
    ...
    for (let i = 0; i <= totalSteps; i++) {
      ...
      const astroTime = (Astronomy as any).MakeTime(currentTime);
      const position = (Astronomy as any).Equator(body, astroTime, this.observer, true, true);
  ...
  calculateRiseTransitSet(
    ...
    const astroTime = (Astronomy as any).MakeTime(date);
    
    try {
      const rise = (Astronomy as any).SearchRiseSet(body, this.observer, 1, astroTime, 1);
      const set = (Astronomy as any).SearchRiseSet(body, this.observer, -1, astroTime, 1);
      const transit = (Astronomy as any).SearchHourAngle(body, this.observer, 0, astroTime);
